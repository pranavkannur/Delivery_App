...
const MapRecenter: React.FC<{ center: [number, number]; zoom?: number }> = ({
  center,
  zoom = 14,
}) => {
  const map = useMap();
...
}> = ({ onLocationSelect, isInteractive }) => {
  useMapEvents({
    click(e) {
      if (isInteractive) {
...
export const CustomerDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"EXPLORE" | "CUSTOM_PIN">(
    "EXPLORE",
  );
  ...
  const [addressInput, setAddressInput] = useState(
    "Select address on map or click My Location",
  );
  ...
  useEffect(() => {
    ...
    const startLat = driverLocation
      ? driverLocation.lat
      : selectedOrder.pickupLat;
    const startLng = driverLocation
      ? driverLocation.lng
      ...
      if (data.orderId === selectedOrder.id) {
        ...
        if (data.status === "ACCEPTED") {
          setVisualAlert({
            ...
            message: `${data.driver?.name || "Driver"} is on their way to pick up your order!`,
            type: "info",
          });
          ...
          setVisualAlert({
            ...
            message:
              "Your package is now in transit! Watch the live map for arrival.",
            type: "info",
          });
        } else if (data.status === "DELIVERED") {
          setVisualAlert({
            title: "🎉 ORDER DELIVERED",
            message: "Delivery completed successfully! Handover confirmed.",
            type: "success",
          });
        }
      }
  ...
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === "CASH_ON_DELIVERY") {
      try {
        ...
        setCart([]);
        alert(
          "📦 Order Placed with Cash on Delivery! Pay the driver upon delivery.",
        );
      } catch (err: any) {
            ...
            {activeTab === "EXPLORE" && isCreatingOrder && (
              <div className="bg-[#f8f8f9] border border-black rounded-2xl p-6 shadow-md space-y-4">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 text-[10px]">
                  {["ALL", "Bakery", "Pizza", "Café", "Groceries"].map(
                    (cat) => (
                      <button
                        key={cat}
                        ...
                        className={`px-2.5 py-1 rounded-lg font-bold uppercase transition shrink-0 ${
                          selectedCategory === cat
                            ? "bg-black text-white"
                            : "bg-[#f0f0f2] text-[#71717a] border border-[#e4e4e7]"
                        }`}
                      >
                        {cat}
                      </button>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <span
                                className={`text-[10px] block ${selectedStore?.id === s.id ? "text-[#d4d4d8]" : "text-[#71717a]"}`}
                              >
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      ...
                      selectedStore.menu.length === 0 ? (
                        <p className="text-xs text-[#a1a1aa] py-4 text-center">
                          THIS STORE HAS NOT ADDED ITEMS YET
                        </p>
                      ) : (
                ...
                {orders.length === 0 ? (
                  ...
                  orders.map((ord) => (
                    ...
                    >
                      ...
                      <p
                        className={`text-xs truncate ${selectedOrder?.id === ord.id && !isCreatingOrder ? "text-[#d4d4d8]" : "text-[#5D5F5F]"}`}
                      >
                        📍 {ord.deliveryAddress}
                      </p>
                      ...
                      <div className="flex items-center justify-between text-xs font-bold mt-2 pt-2 border-t border-[#e4e4e7]/30">
                        ...
                        {ord.deliveryOtp && (
                          <span
                            className={`${selectedOrder?.id === ord.id && !isCreatingOrder ? "text-white" : "text-black"}`}
                          >
                            PIN: {ord.deliveryOtp}
                          </span>
                        )}
              ...
              {selectedOrder && !isCreatingOrder ? (
                ...
                <div className="flex items-center justify-between p-2 bg-[#f0f0f2] rounded-xl border border-[#e4e4e7] text-xs">
                  <span className="text-black font-bold">
                    📍{" "}
                    {selectedStore
                      ? `Ordering from: ${selectedStore.name}`
                      : "Click map to choose doorstep pin"}
                  </span>
                  <span className="text-[10px] bg-black text-white px-2 py-0.5 rounded font-bold">
                    PRECISION MAP
                  </span>
              ...
              <div className="h-[520px] w-full rounded-xl overflow-hidden border border-[#e4e4e7] relative shadow-inner">
                ...
                >
                  ...
                  >
                    <Popup>
                      🏬 Store:{" "}
                      {selectedStore ? selectedStore.name : pickupAddress}
                    </Popup>
                  </Marker>
